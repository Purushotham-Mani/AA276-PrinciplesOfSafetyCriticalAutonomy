...
class ValueFunction:
    def __init__(self):
        ...
        yn = None
        if os.path.exists("hj_reachability_values_2_1.npz"):
            yn = (
                input(
                    "Existing hj_reachability_values_2_1.npz file found from a previous solve; use it (Y/n)? "
                )
                .lower()
                .strip()
            )
        if yn is None or yn == "n":
            ...
            print("Saving the value function to hj_reachability_values_2_1.npz.")
            np.savez("hj_reachability_values_2_1.npz", values=self.values)
        else:
            print("Loading previously computed value function from hj_reachability_values_2_1.npz.")
            self.values = np.load("hj_reachability_values_2_1.npz")["values"]
...
class ControllerA:
    ...
    def nominal_control(self, state):
        l = self.V.dynamic.l
        mp = self.V.dynamic.mp
        mc = self.V.dynamic.mc
        g = self.V.dynamic.g
        if state[0]>np.pi/2 and state[0]<np.pi-2.3e-1:
            # return (-l*(mc+mp*np.sin(state[0])**2)-(mc+mp)*g*np.sin(state[0])-mp*l*state[1]**2*np.sin(state[0])*np.cos(state[0]))/np.cos(state[0])
