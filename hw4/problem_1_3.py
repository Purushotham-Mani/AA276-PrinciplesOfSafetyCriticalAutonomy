...
class ValueFunction:
    def __init__(self):
        ...
        if yn is None or yn == "n":
            ...
            np.savez("hj_reachability_values.npz", values=self.values)
        else:
            print("Loading previously computed value function from hj_reachability_values.npz.")
            self.values = np.load("hj_reachability_values.npz")["values"]
...
def plot_value_and_safe_set_boundary(values_converged, grid, ax):
    ...
    vbar=3
    vis_thetas = np.linspace(0, 2*np.pi, num=101, endpoint=True)
...
plot_value_and_safe_set_boundary(V.values[10], V.grid, ax)
plt.show()