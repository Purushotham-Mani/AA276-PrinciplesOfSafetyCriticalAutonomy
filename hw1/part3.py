...
def plot_h(fig, ax, px, py, slice, h_fn):
    ...
    h = h_fn(X.reshape(-1, 13)).reshape(len(px), len(py)).detach().cpu().numpy()
    colormap = ax.pcolormesh(PX, PY, h, shading='auto', cmap='viridis')
    
    fig.colorbar(colormap, ax=ax, label='CBF value')

    contour = ax.contour(PX, PY, h, levels=[0.0], colors='red', linewidths=2)
    ax.clabel(contour, fmt='%.1f', colors='red', fontsize=10)
...
def plot_and_eval_xts(fig, ax, x0, u_ref_fn, h_fn, dhdx_fn, gamma, lmbda, nt, dt):
    ...
    for i in range(xts.shape[0]):
        ax.plot(xts[i, :, 0].cpu().numpy(), xts[i, :, 1].cpu().numpy(), alpha=0.5)

    safe_mask_ = safe_mask(x0)
    failure_mask_ = failure_mask(xts[:, -1, :])
    false_safety_rate = torch.sum(safe_mask_ & failure_mask_).item() / torch.sum(safe_mask_).item()
